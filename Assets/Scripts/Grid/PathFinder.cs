...
public class PathFinder {
    ...
    public List<OverlayTile> FindPath(OverlayTile start, OverlayTile end, List<OverlayTile> inRangeTiles) {
        ...
        if (inRangeTiles.Count > 0) {
            foreach (var item in inRangeTiles) {
                searchableTiles.Add(item.grid2DLocation,
                    ((NetworkManagerHandler)NetworkManager.singleton).map[item.grid2DLocation]);
            }
        }
        else {
            searchableTiles = ((NetworkManagerHandler)NetworkManager.singleton).map;
        }
    ...
    private List<OverlayTile> GetNeightbourOverlayTiles(OverlayTile currentOverlayTile) {
        var map = ((NetworkManagerHandler)NetworkManager.singleton).map;
